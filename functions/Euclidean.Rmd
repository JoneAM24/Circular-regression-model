...
np.est<-function(x,y,t,bw,type,kertype){
  n <- length(y)
  r1<-npreg(txdat=x,tydat=as.numeric(sin(y)),exdat=t,bws=bw,regtype=type,ckertype=kertype)
  p1<-fitted(r1)
  r2<-npreg(txdat=x,tydat=as.numeric(cos(y)),exdat=t,bws=bw,regtype=type,ckertype=kertype)
  p2<-fitted(r2)
  mhat <- atan2(p1,p2)
...
bw.circ.cv<- function(x,y,type="ll",kertype="epanechnikov",lower=0,upper=2,tol=1e-2){
  n<-length(y)
  err<-numeric(n)
  cv<-function(bw){
    for (i in 1:n){
      err[i]<-1-cos(y[i]-np.est(x[-i],y[-i],x[i],bw,type,kertype))
    }
    return(sum(err))
  }
...
bw.circ.case<- function(x,y,trend,type="ll",kertype="epanechnikov",lower=0,upper=2,tol=1e-2){
  n<-length(y)
  err<-numeric(n)
...
est.circ<-function(x,y,t,bw=NULL,type="ll",kertype="epanechnikov"){
  datax <- x
	...
	n <- length(y)
	if (is.null(bw)){
		bw <- bw.circ.cv(x,y,type,kertype,lower=0,upper=2,tol=1e-2)	
	}
