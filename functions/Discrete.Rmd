...
np.est<-function(x,y,t,bw,type,kertype){
  n <- length(y)
  r1<-npreg(txdat=x,tydat=as.numeric(sin(y)),exdat=t,bws=bw,regtype=type,okertype=kertype)
  p1<-fitted(r1)
  r2<-npreg(txdat=x,tydat=as.numeric(cos(y)),exdat=t,bws=bw,regtype=type,okertype=kertype)
  p2<-fitted(r2)
  mhat <- atan2(p1,p2)
